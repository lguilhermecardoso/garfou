...
| allowCustomization | boolean       | enables modifier groups on digital menu          |
| allowSplit         | boolean       | enables 2/3/4-part split flow                    |
| maxSplits          | int           | fixed number of parts allowed for split products |
...
Structured customization groups per product.
...
| restaurantId  | uuid      | FK                                                 |
| productId     | uuid      | FK products                                        |
| name          | varchar   | e.g. "Ingredientes", "Adicionais"                  |
| type          | enum      | INGREDIENT, ADDON, REQUIRED_CHOICE, REQUIRED_MULTI |
| minSelections | int       | minimum selections required                        |
| maxSelections | int       | maximum selections allowed                         |
| sortOrder     | int       | display order                                      |
| createdAt     | timestamp |                                                    |
| updatedAt     | timestamp |                                                    |
| deletedAt     | timestamp | soft delete                                        |
...
Options inside a modifier group.

| Column       | Type          | Notes                                 |
| ------------ | ------------- | ------------------------------------- |
| id           | uuid          | PK                                    |
| restaurantId | uuid          | FK                                    |
| groupId      | uuid          | FK modifier_groups                    |
| name         | varchar       | e.g. "Sem cebola", "Extra cheddar"    |
| price        | decimal(10,2) | 0 for ingredients, >0 for paid extras |
| isDefault    | boolean       | present by default in the product     |
| isAvailable  | boolean       | temporary availability toggle         |
| sortOrder    | int           | display order                         |
| createdAt    | timestamp     |                                       |
| updatedAt    | timestamp     |                                       |
| deletedAt    | timestamp     | soft delete                           |

### product_split_flavors

Explicit pool of flavors allowed for a split-capable product.

| Column          | Type      | Notes                           |
| --------------- | --------- | ------------------------------- |
| id              | uuid      | PK                              |
| restaurantId    | uuid      | FK                              |
| sourceProductId | uuid      | FK products — base product      |
| flavorProductId | uuid      | FK products — selectable flavor |
| sortOrder       | int       | display order                   |
| isAvailable     | boolean   | temporary availability toggle   |
| createdAt       | timestamp |                                 |
| updatedAt       | timestamp |                                 |
...
Snapshot of structured customization choices made on an item.

| Column      | Type          | Notes                                         |
| ----------- | ------------- | --------------------------------------------- |
| id          | uuid          | PK                                            |
| orderItemId | uuid          | FK order_items                                |
| optionId    | uuid          | FK modifier_options                           |
| quantity    | int           |                                               |
| unitPrice   | decimal(10,2) | snapshot                                      |
| optionName  | varchar       | snapshot for historical integrity             |
| isRemoval   | boolean       | true when client removed a default ingredient |
| createdAt   | timestamp     |                                               |

### order_item_splits

Snapshot of each part/flavor selected for a split product.

| Column      | Type          | Notes                 |
| ----------- | ------------- | --------------------- |
| id          | uuid          | PK                    |
| orderItemId | uuid          | FK order_items        |
| splitIndex  | int           | 0-based part index    |
| productId   | uuid          | FK products           |
| productName | varchar       | flavor name snapshot  |
| unitPrice   | decimal(10,2) | flavor price snapshot |
| createdAt   | timestamp     |                       |
