...
/**
 * KitchenAppActivate - Tela de ativação por TOKEN para App Cozinha (BFF)
 *
 * Fluxo:
 * 1. Gerente visualiza TOKEN no dashboard (6 dígitos, permanente)
 * 2. Abre esta tela em TV/tablet da cozinha
 * 3. Cozinheiro digita TOKEN
 * 4. Sistema valida via BFF e retorna bearerToken
 * 5. Armazena bearerToken no localStorage
 * 6. Redireciona para /kitchen-app (fullscreen)
 *
 * Segurança: Token é permanente, pode ser usado por múltiplas TVs/tablets, vinculado ao restaurante
 * BFF: Independente de cookies/NextAuth, funciona em abas anônimas
 */
export default function KitchenAppActivate() {
  ...
  const validateToken = useCallback(
    async (fullToken: string) => {
      ...
      try {
        ...
        router.push(`/kitchen-app/${data.data.restaurant.slug}`);
      } catch (err) {
    ...
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-orange-50 to-red-100 p-4">
      <Card className="w-full max-w-md p-8 shadow-2xl">
        <div className="mb-8 text-center">
          <div className="mx-auto mb-4 flex h-20 w-20 items-center justify-center rounded-full bg-orange-600">
            <svg
          ...
          <h1 className="mb-2 text-3xl font-bold text-gray-900">App Cozinha</h1>
        ...
        {loading && (
          <div className="mb-4 text-center text-gray-600">
            <div className="mb-2 inline-block h-6 w-6 animate-spin rounded-full border-b-2 border-orange-600"></div>
            <p>Validando token...</p>
          </div>
        )}
        ...
        <div className="space-y-1 text-center text-sm text-gray-500">
          ...
          <p>✓ Pode ser usado em múltiplas TVs</p>
