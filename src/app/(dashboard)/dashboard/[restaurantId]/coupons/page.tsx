    ...
    <div className="space-y-6">
      ...
      {coupons.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-2xl bg-white py-20 shadow-sm">
          ...
          <a href={`/dashboard/${restaurantId}/coupons/new`} className="mt-4 text-sm font-semibold text-primary-500 hover:underline">
                ...
                return (
                  <tr key={c.id} className="border-b border-neutral-50 hover:bg-neutral-50">
                    <td className="px-4 py-3">
                      <code className="rounded bg-neutral-100 px-1.5 py-0.5 font-mono text-sm font-semibold text-neutral-900">{c.code}</code>
                    </td>
                    <td className="px-4 py-3 text-neutral-500">
                      {c.type === "PERCENTAGE" ? "Porcentagem" : "Valor fixo"}
                    </td>
                    <td className="px-4 py-3 text-right font-semibold text-neutral-900">
                      {c.type === "PERCENTAGE" ? `${Number(c.value)}%` : formatCurrency(Math.round(Number(c.value) * 100))}
                    </td>
                    ...
                    <td className="px-4 py-3 text-center">
                      <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${active ? "bg-emerald-50 text-emerald-700" : "bg-neutral-100 text-neutral-500"}`}>
